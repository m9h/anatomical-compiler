...
- [x] **bioelectric-layer companion: a JAX/`diffrax` refactor of BETSE/BETSEE** (sister project at `~/Workspace/betse-unified` — done & integrated as a referenced companion; Pietak & Levin 2016/2017 finite-volume Vmem/gap-junction/ion-channel solver, made differentiable) — follows the Levin Lab / Mafe-group post-2018 shift from passive simulation to *agential design*: (i) inverse bioelectric design (`jax.grad` to a target shape — the xenobot move by gradient descent; Kriegman 2020/2021), (ii) bioelectric prepattern → triggers a secondary GRN (Pietak & Levin 2017; Cervera/Levin/Mafe 2024–2026 top-down Vmem↔transcription), (iii) morphoceutical intervention timelines (drug-cocktail schedule → limb regrowth; Murugan 2022; Pio-Lopez & Levin 2023), (iv) large-scale pattern integration / embryonic-brain prepattern (Manicka, Pai & Levin 2023). Each is one optimal-control problem on a differentiable plant — the bioelectric set-point added to the §4.3 actuator menu, with a real solver behind it. Manifesto: Levin 2021 (Cell). Refs 38, 39a–39e in `REFERENCES.md`; the natural Lab-9 companion (notebooks/README "Bioelectric-layer simulators").
  - [x] JAX backend proxy (`BETSE_JAX=1`), functional pytree state (`SimState`/`CellData`/`SimParams`), pure JIT-compatible physics solvers — `update_v_pure` (Vmem), `update_gj_pure` (gap junctions), `update_conc_pure` (ion concentrations) — and a whole-step JIT-fused `step_pure` (`solver/stepper.py`, ~2.2 µs/step for 50 cells via `lax.scan`) + a GPU-accelerated Poisson solver; `jax.grad` end-to-end; 100% test coverage on the JAX modules. Docs: `docs/JAX_REFAC.md`.
  - [x] **inverse bioelectric design** — `betse.science.jax.inverse.optimize_pattern` (gradient descent through the differentiable solvers, MSE to a target Vmem pattern). Verified: `test_jax_inverse.py` drives a resting 10-cell tissue into an alternating depolarised(−20 mV)/hyperpolarised(−80 mV) pattern, loss 9e-4 → **1.36e-10**. The bioelectric instance of "the anatomical compiler" (target pattern in → required ion currents out).
  - [x] **xenobot motility** — `betse.science.jax.physics.motility` (bioelectric force → movement); `test_jax_xenobot.py`. The Kriegman 2020/2021 design move by `jax.grad` on the cell-collective's force/motility goals rather than by an evolutionary search.
...
`notebooks/02_regulomes_and_hypergraphs.ipynb`, `scripts/organoid_hgx_colab.ipynb`, the three
