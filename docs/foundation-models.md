...
## Concrete pipeline example — Lab 11 design

A new `notebooks/11_foundation_model_pipeline.ipynb`, slotting into the existing flow:
...
5. **Real-mode validation on a Lab 3 dataset** — point [`scripts/fm_embed.py`](../scripts/fm_embed.py) and [`scripts/fm_edges_seq.py`](../scripts/fm_edges_seq.py) at the Pollen brain-organoid or fetal-kidney h5ad (plus a Fleck-style edges CSV + hg38 promoters FASTA), `--mode real`. Driver [`scripts/run_fm_real_dgx.sh`](../scripts/run_fm_real_dgx.sh) orchestrates the full set; setup in [`docs/dgx-spark-setup.md`](dgx-spark-setup.md). ~1 hour wall-clock on DGX Spark (128 GB GPU). **Ready to run** as soon as a real h5ad + edges + promoters are in place.

Steps 1–3 are done; step 4 (scGPT in-silico KD as BO signal) remains, ~1 week. Step 5 (real-mode measurement on the project's actual benchmarks) is **infrastructure-ready** — needs only the real-data inputs.
