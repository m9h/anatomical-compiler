...
\begin{frame}{Conceptual Foundation: Synthetic Morphogenesis}

\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The IEEE 2023 Intro Material}
\begin{itemize}
    \item \textbf{Jamie A. Davies}: "Programming living mammalian cells to make structures"
    \item \textbf{Biology as Software}: Genetic circuits program the logic
    \item \textbf{Biology as Hardware}: Cellular shape-generating processes (adhesion, motility)
\end{itemize}

\vspace{1em}
\textbf{The Quantitative Challenge}
\begin{itemize}
    \item How do we verify that the programmed logic matches primary tissue?
    \item Need metrics for \textbf{Fidelity}, \textbf{Modularity}, and \textbf{Topology}.
\end{itemize}
\end{column}

\begin{column}{0.48\textwidth}
\begin{tikzpicture}[scale=0.9]
\node[draw, fill=darkbg, text=white, rounded corners, inner sep=5pt] (soft) at (0,1.5) {Software (GRN Logic)};
\node[draw, fill=hgxblue!20, rounded corners, inner sep=5pt] (hard) at (0,-1.5) {Hardware (Morphogenesis)};
\draw[<->, ultra thick, dashed, mygray] (soft) -- (hard);
\node[font=\scriptsize, align=center] at (1.8,0) {This Work:\\Quantifying the\\Link with \hgx};
\end{tikzpicture}
\end{column}
\end{columns}
...
\begin{frame}{Microglia-Driven Maturation}

\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The "Maturation Module" (Park 2023)}
\begin{itemize}
    \item Comparing organoids with and without integrated microglia
    \item Microglia-derived \textbf{cholesterol} drives neurogenesis
\end{itemize}

\vspace{1em}
\textbf{hgx Benchmark Result}
\begin{itemize}
    \item Significant increase in \textbf{Sonthalia Pattern 2} (Mature Neuron) activity
    \item Maturity Shift: \textbf{+0.06}, \textbf{$p = 2.65 \times 10^{-11}$}
    \item Quantifies immune-neural cross-talk as a requirement for fidelity
\end{itemize}
\end{column}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{microglia_park_maturation.png}
\caption{Maturity shift in microglia co-cultures}
\end{figure}
\end{column}
\end{columns}
...
\begin{itemize}
    \item \textbf{Synaptic Resilience}: \textbf{9.8$\times$} increase in Synaptic (ASD) activity in bioprinted brain (\textit{Tang 2020, Shin 2024})
    \item \textbf{oRG Expansion}: \textbf{9.0$\times$} increase in human-specific oRG programs (\textit{Vassal 2024})
\end{itemize}

\vspace{1em}
\textbf{The Biofabrication Advantage}
\begin{itemize}
    \item Bioprinting "unlocks" mature regulatory architectures missing in organoids
    \item Enables modeling of late-stage disease (\textit{Early-Stage Buffer})
\end{itemize}
\end{column}
...
\begin{frame}{Quantifying Modularity: NITMB Framework}

\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The Identifiability Challenge}
\begin{itemize}
    \item NITMB highlights the need to distinguish discrete "control units" (modules)
    \item We implemented a \textbf{Module Identifiability Index} based on the Hodge $L_0$ spectral gap
\end{itemize}

\vspace{1em}
\textbf{Cross-System Benchmark}
\begin{itemize}
    \item \textbf{Organoid}: 0.38 (High distinctness)
    \item \textbf{Fetal Kidney}: 0.37 (Primary baseline)
    \item \textbf{Bioprinted}: 0.35 (Nearing primary-like modularity)
\end{itemize}
\end{column}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{nitmb_modularity_test.png}
\caption{Module Identifiability Scores}
\end{figure}
\end{column}
\end{columns}
...
\begin{frame}{A Quantitative Framework for Synthetic Morphology}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{System} & \textbf{Technology} & \textbf{Davies Module} & \textbf{Dataset} \\ \midrule
\rowcolor{hgxblue!10} Self-Organized & Brain Organoids & \textbf{Sensor/Feedback} & Fleck 2023 \\
\rowcolor{pollengreen!10} Biofabricated & 3D Bioprinting & \textbf{Scaffold/Support} & Tang 2020 \\
\rowcolor{chooseviolet!10} Programmed & synNotch Circuits & \textbf{Regulatory Logic} & Toda 2020 \\
\rowcolor{bonfsig!10} Embodied & Anthrobots & \textbf{Effector/Motility} & Gumuskaya 2023 \\ \bottomrule
\end{tabular}
\end{table}

\begin{itemize}
    \item \textbf{Regulatory}: synNotch programs stable \textit{"Hypergraph States"}
    \item \textbf{Effector}: FOXJ1 regulon drives \textit{physical cilia assembly}
    \item \textbf{Sensor}: Hodge Laplacians detect \textit{maturation stop-signals}
\end{itemize}
...
\begin{enumerate}
    ...
    \vspace{0.4em}
    \item \textbf{Microglia as Maturation Modules}\\
    {\small Park 2023: \textbf{Maturity shift ($p=5.0 \times 10^{-15}$)}. Popova 2021: Chimeric integration \textbf{increases regulatory modularity} (5 $\rightarrow$ 8 clusters).}
...
\centering
\textbf{Code}: \texttt{github.com/m9h/anatomical-compiler}\\
...
\vspace{1em}
\normalsize
\texttt{github.com/m9h/anatomical-compiler}
\end{frame}
